...
public class JsGridReportBase {
    ...
    public void exportToExcel(String title, String creator, TableData tableData)
        ...
        if (request.getHeader("User-Agent").toLowerCase().indexOf("msie")>0||flag){
        	...
        	title = new String(title.replaceAll(" ", "").getBytes("UTF-8"), "ISO8859-1");
            //firefox浏览器
            //firefox浏览器User-Agent字符串: 
            //Mozilla/5.0 (Windows NT 6.1; WOW64; rv:36.0) Gecko/20100101 Firefox/36.0
        }
        response.setHeader("Content-Disposition", "attachment;filename=".concat(title).concat(".xls;"));
//                .concat(new String(sFileName.getBytes("gbk"), "iso-8859-1")));//String.valueOf(URLEncoder.encode(sFileName, "UTF-8"))
    ...
    private HashMap<String, HSSFCellStyle> initStyles(HSSFWorkbook wb) {
        ...
        try {
        	File template =  new File(session.getServletContext().getRealPath("/")+MODULE_PATH);
        	if(template.exists()){
    			...
    			System.out.println("找不到excel导出模板");
    		}
        } catch (Exception e) {
            e.printStackTrace();
        }
