...
public class ExcelUtils {
    ...
    public static void writeFile2Browser(File file, HttpServletRequest req, HttpServletResponse res) throws Exception {
        if (file == null || req == null || res == null)
            throw new NullPointerException();
        if (!file.isFile())
            throw new IOException();
        String fileName = file.getName();
        FileInputStream pi = new FileInputStream(file);
        PrintStream ps = new PrintStream(res.getOutputStream());
        //判断是否是IE11
        Boolean flag= req.getHeader("User-Agent").indexOf("like Gecko")>0;
        ...
        if (req.getHeader("User-Agent").toLowerCase().indexOf("msie")>0||flag){
        ...
        res.reset();// 清除首部空白行
        res.setContentType(MediaType.parseMediaType("application/vnd.ms-excel").toString());
        res.setHeader("Content-Disposition", "attachment;filename=".concat(fileName));
        res.setHeader("Connection", "close");
        res.setHeader("Content-Type", "application/vnd.ms-excel");
        byte[] buff = new byte[1024];
        int len;
        while ((len = pi.read(buff)) != -1)
            ps.write(buff, 0, len);
        pi.close();
        ps.flush();
        ps.close();
